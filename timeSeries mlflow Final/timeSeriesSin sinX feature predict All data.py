...
import mlflow 
mlflow.autolog()
mlflow.set_experiment("sinX feature approximate + predict new data FINAL")
def generateSin():
...
if __name__ == "__main__":
    with mlflow.start_run(run_name="good run"):
        ...
        plt.plot(expandedData['x'], predictedY, c='b', linewidth = 1, label ="approximation + prediction on new data")
        plt.legend()
        plt.subplot(212)
        plt.plot(x, y, c = 'm', label = "original data")
        plt.legend()
        mlflow.log_figure(plt.gcf(), "approximation + prediction on new data.png")
